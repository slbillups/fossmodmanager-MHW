...
mod tests {
    ...
    async fn test_setup_flow() -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(
            initial_config.is_none(),
            "Expected no initial config to exist"
        );
        ...
        assert!(
            loaded_config.is_some(),
            "Expected config to be loaded after saving"
        );

        let loaded_data = loaded_config.unwrap();
        // Check fields based on your actual GameData struct
        assert_eq!(
            loaded_data.game_executable_path,
            mock_game_data.game_executable_path
        );
